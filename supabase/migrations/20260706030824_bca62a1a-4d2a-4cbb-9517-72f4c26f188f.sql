...
-- Introduce a policy start date so SLA fines only start accruing from today (2026-07-06 KST).
-- Any deadline before the policy start is clamped to the policy start for fine calculation.

CREATE OR REPLACE FUNCTION public.sla_policy_start()
RETURNS timestamptz
LANGUAGE sql IMMUTABLE
SET search_path = public
AS $$ SELECT '2026-07-05 15:00:00+00'::timestamptz $$;  -- 2026-07-06 00:00 KST
...
AS $function$
  ...
  SELECT c.id, c.name, c.phone, c.country_id, c.country_code, c.status, c.since_ts, c.deadline,
    ...
    GREATEST(0, CEIL(EXTRACT(EPOCH FROM (now() - GREATEST(c.deadline, p.start_ts)) ) / 86400))::int AS overdue_days,
    c.daily_fine,
    (GREATEST(0, CEIL(EXTRACT(EPOCH FROM (now() - GREATEST(c.deadline, p.start_ts))) / 86400))::int * c.daily_fine)::int AS fine_total,
    c.assigned_to
  FROM calc c CROSS JOIN policy p
  ...
  win AS (
    SELECT b.id, b.country_id, b.status, b.daily_fine,
      GREATEST(0, CEIL(EXTRACT(EPOCH FROM (
        ...
        - GREATEST(b.deadline, _period_start::timestamptz, p.start_ts)
