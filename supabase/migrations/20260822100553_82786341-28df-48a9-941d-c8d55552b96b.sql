...
BEGIN
  ...
  UPDATE public.telegram_chats
     SET reply_lock_by = _me,
         reply_lock_at = now(),
         assigned_operator_id = COALESCE(assigned_operator_id, _me)
   WHERE id = _chat_row_id
     AND (
       reply_lock_by IS NULL
       OR reply_lock_by = _me
       OR reply_lock_at IS NULL
       OR reply_lock_at < now() - interval '120 seconds'
  ...
  SELECT reply_lock_by,
         GREATEST(0, 120 - EXTRACT(EPOCH FROM (now() - reply_lock_at))::int)
    INTO _holder, _left
