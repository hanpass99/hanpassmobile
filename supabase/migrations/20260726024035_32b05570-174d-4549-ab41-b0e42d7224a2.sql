...
AS $function$
  WITH policy AS (
    SELECT DATE '2026-07-24' AS start_date,
           DATE '2026-07-27' AS checkin_start_date
  ),
  bounds AS (
    SELECT
  ...
  joined AS (
    SELECT
      ...
      CASE
        WHEN m.day >= (SELECT checkin_start_date FROM policy)
          THEN (a.status IS NULL OR a.status <> 'present')
        ELSE (a.status IS NOT NULL AND a.status <> 'present')
      END AS is_absent,
