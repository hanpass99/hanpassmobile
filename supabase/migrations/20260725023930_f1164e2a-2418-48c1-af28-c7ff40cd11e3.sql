...
-- 1) Per-staff daily call goal
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS daily_call_goal integer NOT NULL DEFAULT 50;

-- 2) Admin setter (goal)
CREATE OR REPLACE FUNCTION public.admin_set_call_goal(_user_id uuid, _goal integer)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  IF _goal IS NULL OR _goal < 0 THEN
    RAISE EXCEPTION 'invalid goal';
  END IF;
  UPDATE public.profiles SET daily_call_goal = _goal, updated_at = now()
   WHERE id = _user_id;
  RETURN _goal;
END $$;

REVOKE ALL ON FUNCTION public.admin_set_call_goal(uuid, integer) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.admin_set_call_goal(uuid, integer) TO authenticated;

-- 3) Extend the fines report: focus_date + per-staff goal
DROP FUNCTION IF EXISTS public.sla_staff_call_fines(date, date, integer, integer);
DROP FUNCTION IF EXISTS public.sla_staff_call_fines(date, date, integer, integer, date);
...
CREATE OR REPLACE FUNCTION public.sla_staff_call_fines(
  ...
  _focus_date date DEFAULT CURRENT_DATE
)
RETURNS TABLE(
  user_id uuid,
  display_name text,
...
AS $$
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE NOT s.is_absent AND NOT s.is_waived AND s.calls < s.goal)::int AS days_under,
    ...
    COALESCE(SUM(s.calls) FILTER (WHERE s.day = _focus_date), 0)::bigint AS focus_calls,
    COALESCE(bool_or(s.fined) FILTER (WHERE s.day = _focus_date), false) AS focus_fined,
    COALESCE(bool_or(s.is_waived) FILTER (WHERE s.day = _focus_date), false) AS focus_waived,
...
GRANT EXECUTE ON FUNCTION public.sla_staff_call_fines(date, date, integer, integer, date) TO authenticated;
