...
AS $function$
  ...
  SELECT c.id, c.name, c.phone, c.country_id, c.country_code, c.status, c.since_ts, c.deadline,
    ...
    GREATEST(0, FLOOR(EXTRACT(EPOCH FROM (now() - GREATEST(c.deadline, p.start_ts))) / 86400))::int AS overdue_days,
    c.daily_fine,
    (GREATEST(0, FLOOR(EXTRACT(EPOCH FROM (now() - GREATEST(c.deadline, p.start_ts))) / 86400))::int * c.daily_fine)::int AS fine_total,
    c.assigned_to
