CREATE OR REPLACE FUNCTION public.stats_hanpass_dashboard(_date_from timestamp with time zone, _date_to timestamp with time zone, _country_ids uuid[] DEFAULT NULL::uuid[], _staff_ids uuid[] DEFAULT NULL::uuid[], _pools text[] DEFAULT NULL::text[])
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  team_agg AS (
    SELECT t.country_id,
           ...
           COUNT(*) AS handled,
           COUNT(*) FILTER (WHERE t.status = 'activated') AS activated
  ...
  staff_agg AS (
    SELECT e.staff_id,
           ...
           COUNT(*) AS handled,
           COUNT(*) FILTER (WHERE e.status = 'activated') AS activated,
  ...
  SELECT jsonb_build_object(
    ...
    'prev_totals', (SELECT jsonb_build_object(
        'handled', COUNT(*), 'customers', COUNT(DISTINCT customer_id),
        'activated', COUNT(*) FILTER (WHERE status = 'activated')) FROM ev_prev),
