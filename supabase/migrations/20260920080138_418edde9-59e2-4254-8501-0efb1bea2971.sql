...
BEGIN
  ...
  SELECT COUNT(*) INTO _n2 FROM public.customers
   WHERE assigned_to IS NULL AND status = 'new' AND created_at > now() - interval '7 days';
  INSERT INTO public.system_checks (check_key, category, status, message, metric, details)
  VALUES ('unassigned_customers', 'data',
    CASE WHEN _n2 = 0 THEN 'ok' WHEN _n2 < 500 THEN 'warn' ELSE 'error' END,
    CASE WHEN _n2 = 0 THEN format('최근 7일 신규 고객은 모두 담당자 배정됨 (전체 미배정 %s명)', _n)
         ELSE format('최근 7일 담당자 미배정 신규 고객 %s명 (전체 미배정 %s명)', _n2, _n) END,
    _n2, jsonb_build_object('total_unassigned', _n, 'recent_unassigned', _n2));
  ...
  VALUES ('duplicate_phone', 'data',
    CASE WHEN _n = 0 THEN 'ok' WHEN _n < 1000 THEN 'warn' ELSE 'error' END,
  ...
  VALUES ('google_sheet_sync', 'integration',
    ...
    round(EXTRACT(EPOCH FROM (now() - COALESCE(_ts, now()))) / 3600, 1),
