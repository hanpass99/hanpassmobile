...
AS $function$
  ...
  SELECT c.id, c.name, c.phone, c.country_id, c.country_code, c.status, c.since_ts, c.deadline,
    ...
    c.daily_fine,
    GREATEST(0, ROUND(EXTRACT(EPOCH FROM (now() - c.deadline)) / 3600 * c.daily_fine / 24.0))::int AS fine_total,
    c.assigned_to
  FROM calc c
...
AS $function$
  ...
  win AS (
    SELECT b.id, b.country_id, b.status, b.daily_fine,
      ...
      )) / 3600)::numeric AS hours_in_window,
  ...
  agg AS (
    SELECT country_id,
      ...
      COALESCE(ROUND(SUM(hours_in_window * daily_fine / 24.0)), 0)::bigint AS gross
    FROM win
    GROUP BY country_id
  ),
  adj AS (
